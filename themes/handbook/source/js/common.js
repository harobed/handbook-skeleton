(function () {
  ...
  function initHashLevelRedirects() {
    checkForHashRedirect(/components\.html$/, {
      ...
      'Leadership': '/general/leadership.md',
      'Quarterly OKRs': '/general/okrs.md',
  ...
  function initApiSpecLinks () {
    ...
    function createSourceSearchPath (query) {
      query = query
        .replace(/\([^\)]*?\)/g, '')
        .replace(/(handbook\.)(\w+)/g, '$1$2" OR "$2')
        .replace(/vm\./g, 'handbook.prototype.')
      return 'https://github.com/search?utf8=%E2%9C%93&q=repo%3Ahandbookjs%2Fhandbook+extension%3Ajs+' + encodeURIComponent('"' + query + '"') + '&type=Code'
    }
  }
